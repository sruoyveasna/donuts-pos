                ...
                <div class="inline-flex items-center gap-2 text-xs font-semibold uppercase tracking-wide">
                    <span
                        class="flex h-6 w-6 items-center justify-center rounded-full
                               bg-white/10 border border-white/30 shadow-sm shadow-rose-900/40">
                        <i class="bi bi-person-plus text-[13px]"></i>
                    </span>
                    <span>
                        {{ __('messages.users_create_title', [], $locale) ?: 'New user' }}
                    </span>
                </div>
                <p class="mt-0.5 text-[11px] text-rose-50/90">
                    {{ __('messages.users_create_subtitle', [], $locale) ?: 'Create a new staff account.' }}
                </p>
            </div>
        ...
        <form id="createForm" class="px-4 md:px-5 py-4 space-y-4">
                ...
                <div class="space-y-1">
                    ...
                    <input
                        ...
                        class="w-full rounded-xl border border-slate-200/80 dark:border-slate-700/80
                               ...
                               placeholder:text-slate-400 dark:placeholder:text-slate-500
                               focus:outline-none focus:ring-1 focus:ring-rose-300 focus:border-rose-400"
                        placeholder="{{ __('messages.users_field_password_placeholder', [], $locale) ?: 'Minimum 6 characters' }}"
                        required>
                    <p class="text-[11px] text-slate-400 dark:text-slate-500">
                        {{ __('messages.users_field_password_hint', [], $locale) ?: 'User can change password later.' }}
                    </p>
                </div>
            </div>
            ...
            <div class="flex justify-between items-center pt-1">
                ...
                <button
                    ...
                    <span>{{ __('messages.users_button_create', [], $locale) ?: 'Create user' }}</span>
                </button>
            </div>
        </form>
    </div>
